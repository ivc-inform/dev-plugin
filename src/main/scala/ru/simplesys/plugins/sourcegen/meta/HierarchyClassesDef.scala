...
//    println(s"${group}.${className}")
    ...
    (referenceToParent, hierarchySetting) match {
      case (Some(p), Some((lDiscriminatorField, lHierarchyMapping))) =>
        ...
        new ChildHierarchyClassDefData(group, className, classCaption, classIsAbstract, strictAttrs, strictUCs, strictFKs, overrideFKs, defaultSettings, isAutoTableMapping, p) with ChildHierarchyClassDef
      ...
      case (None, Some((lDiscriminatorField, lHierarchyMapping))) => new RootHierarchyClassDefData(group, className, classCaption, classIsAbstract, strictAttrs, strictUCs, strictFKs, defaultSettings, isAutoTableMapping, lHierarchyMapping, lDiscriminatorField) with RootHierarchyClassDef
