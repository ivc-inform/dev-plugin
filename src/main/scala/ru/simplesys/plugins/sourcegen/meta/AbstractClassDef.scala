...
trait AbstractClassDef {
    ...
    def toXml(implicit resolver: SchemaDef): Elem = {
        val allAttrs = attrNames(resolver).map { x => attr(x)(resolver)}

        <class name={className} caption={classCaption} group={group} groupCaption={resolver.groups.filter(_.selfRef === group).head.caption} isAbstract={isAbstract.toString} groupPrefix={resolver.groups.filter(_.selfRef === group).head.prefix}>
            <attrs>
                {allAttrs.map(_.toXML)}
            </attrs>
            <constraints>
                {ucs(resolver).map(_.toXml)}{fks(resolver).map(_.toXml)}
