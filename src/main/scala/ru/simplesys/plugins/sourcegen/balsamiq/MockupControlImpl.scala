        ...
        // we have only DynamicForm or empty canvas
            ...
            childrenShape match {
                ...
                case _ => {
                    ...
                    val xRightConcatenate = xRight.foldLeft(false)((sum, c) => {
                        val (rect, isXRight) = c; sum || isXRight
                    })
                    val yBottomConcatenate = yBottom.foldLeft(false)((sum, c) => {
                        val (rect, isYBottom) = c; sum || isYBottom
                    })
                    val (isVertical, groupedChildren) = (xRightConcatenate, yBottomConcatenate) match {
                        ...
                        case (true, false) => (true, xRight.sortBy { case (rect, isXRight) => rect.y }.foldLeft(Seq.empty[(scala.collection.mutable.ListBuffer[RectangleShape], Boolean)]) { case (seq, (rect, isXRight)) =>
                            ...
                            prev match {
                                case Some(p@(lseq, bval)) => if (bval === isXRight && !isXRight) {
                                    lseq += rect; seq
                                } else seq.:+((scala.collection.mutable.ListBuffer(rect), isXRight))
                        ...
                        case (false, true) => (false, yBottom.sortBy { case (rect, isYBottom) => rect.x }.foldLeft(Seq.empty[(scala.collection.mutable.ListBuffer[RectangleShape], Boolean)]) { case (seq, (rect, isYBottom)) =>
                            val prev = seq.lastOption
                            prev match {
                                case Some(p@(lseq, bval)) => if (bval === isYBottom && !isYBottom) {
                                    lseq += rect; seq
                                } else seq.:+((scala.collection.mutable.ListBuffer(rect), isYBottom))
                    ...
                    val overallLength: Int = childrenWOutSplitters.map(_.length).sum
                    ...
                    val controls = linearChildrenWSplitters.flatten {
                        case (x, splitter, target) =>
                            x.toUIControl {
                                if (isVertical)
                                    CanvasProps(showResizeBar = {
                                        if (splitter) Some(true) else None
                                    }, resizeBarTarget = target, widthPCT = 100.some, heightPCT = math.round(100 * x.length / overallLength).toInt.some, direction = currDirection.some)
                                else CanvasProps(showResizeBar = {
                                    if (splitter) Some(true) else None
                                }, resizeBarTarget = target, widthPCT = math.round(100 * x.length / overallLength).toInt.some, heightPCT = 100.some, direction = currDirection.some)
                            }
                    }
